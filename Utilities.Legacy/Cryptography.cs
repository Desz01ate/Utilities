...
namespace Utilities.Legacy
{
    ...
    {
        public Encryption(string hash,string salt)
        {
            this.Hash = hash;
            this.Salt = salt;
        }
    ...
    public static class Cryptography
    {
        ...
        /// <param name="iterations">Encryption iterations</param>
        /// <returns></returns>
        public static Encryption GenerateHash(string plainText, byte[] salt, int byteSize = 128, int iterations = 2000)
        {
            var plainTextBytes = Encoding.UTF8.GetBytes(plainText);
            using (var rfc2898 = new Rfc2898DeriveBytes(plainTextBytes, salt, iterations))
            {
                var hash = rfc2898.GetBytes(byteSize);
                return new Encryption(Convert.ToBase64String(hash), Convert.ToBase64String(salt));
            }
        }
        /// <summary>
        ...
        /// <param name="iterations">Encryption iterations</param>
        /// <returns></returns>
        public static Encryption GenerateHash(string plainText, int byteSize = 128, int iterations = 2000)
        {
        ...
        /// <param name="iterations">Encryption iterations</param>
        /// <returns></returns>
        public static Encryption GenerateHash(string plainText, string salt, int byteSize = 128, int iterations = 2000)
        {
